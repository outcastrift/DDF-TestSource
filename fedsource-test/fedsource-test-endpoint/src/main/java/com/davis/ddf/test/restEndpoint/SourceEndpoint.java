...
public class SourceEndpoint {
  ...
  private static  ArrayList<String> GEOMETRIES ;
  ...
  public SourceEndpoint() {
    ...
    logger.debug("Arrays Size \n orginatingUnits = " + String.valueOf(originateUnit.size()) +
        " \n summaries = " + String.valueOf(summaries.size()) +
  ...
  public static <T> T getRandomField(T[] geoms) {
    Random random = new Random();
    return geoms[random.nextInt(geoms.length)];
  }
  ...
  public String constructWktString(String wktType,
      Double topLeftLat,
      Double topLeftLng,
      Double bottomRightLat,
      Double bottomRightLng) {
    String result = null;
    if(wktType.equalsIgnoreCase("POLYGON")){
      try {
      result=  createRandomWktPolygon(bottomRightLng, topLeftLat, bottomRightLat, topLeftLng);
      } catch (Exception e) {
        logger.debug("Wkt Creation Failed For POLYGON");
      }
    }else if(wktType.equalsIgnoreCase("MULTILINESTRING")){
      try {
        result=     createRandomWktMultiLine(bottomRightLng, topLeftLat, bottomRightLat, topLeftLng);
      } catch (Exception e) {
        logger.debug("Wkt Creation Failed For MULTILINESTRING");
      }
    }
    else if(wktType.equalsIgnoreCase("POINT")){
      try {
        result=     createRandomWktPoint(bottomRightLng, topLeftLat, bottomRightLat, topLeftLng);
      } catch (Exception e) {
        logger.debug("Wkt Creation Failed For POINT");
      }
    }
    else if(wktType.equalsIgnoreCase("MULTIPOINT")){
      try {
        result=    createRandomWktMultiPoint(bottomRightLng, topLeftLat, bottomRightLat, topLeftLng);
      } catch (Exception e) {
        logger.debug("Wkt Creation Failed For MULTIPOINT");
      }
    }
    else if(wktType.equalsIgnoreCase("MULTIPOLYGON")) {
      try {
        result=     createRandomWktMultiPolygon(bottomRightLng, topLeftLat, bottomRightLat, topLeftLng);
      } catch (Exception e) {
        logger.debug("Wkt Creation Failed For MULTIPOLYGON");
      }
    }
    else if(wktType.equalsIgnoreCase("LINESTRING")){
      try {
        result=      createRandomWktLine(bottomRightLng, topLeftLat, bottomRightLat, topLeftLng);
      } catch (Exception e) {
  ...
  private String createRandomWktMultiPoint(Double topLeftLat,
      Double topLeftLng,
      Double bottomRightLat,
      Double bottomRightLng) {
  ...
  private String createRandomWktPolygon(Double topLeftLat,
      Double topLeftLng,
      Double bottomRightLat,
      Double bottomRightLng) {
    StringBuilder result = new StringBuilder();
  ...
  private String createRandomWktMultiPolygon(Double topLeftLat,
      Double topLeftLng,
      Double bottomRightLat,
      Double bottomRightLng) {
    StringBuilder result = new StringBuilder();
  ...
  private String createRandomWktLine(Double topLeftLat,
      Double topLeftLng,
      Double bottomRightLat,
      Double bottomRightLng) {
    StringBuilder result = new StringBuilder();
  ...
  public Response getResultsForGroovy(@Context UriInfo requestUriInfo,
    ...
    try {
      ...
      if (response != null) {
        ...
        throw new SourceEndpointException("There was a error handling the request.",
  ...
  public Response getResultsForSource(@Context UriInfo requestUriInfo,
    ...
    try {
      ...
      if (response != null) {
        ...
        builder = Response.ok(responseData, MediaType.APPLICATION_JSON)
  ...
  private ArrayList<UniversalFederatedSourceResponse> buildObjectsForSource(
      int amount,
      Double topLeftLat,
      Double topLeftLng,
      Double bottomRightLat,
    ...
    while (amount > 0) {
      ...
      double lng = ThreadLocalRandom.current().nextDouble(topLeftLng, bottomRightLng);
      lng = Double.parseDouble(new DecimalFormat("#.#####").format(lng));

      UniversalFederatedSourceResponse uniResponse =
          generateRandomMetacard(lat, lng,
              topLeftLat,
              topLeftLng,
              bottomRightLat,
              bottomRightLng,
  ...
  private UniversalFederatedSourceResponse generateRandomMetacard(
      double lat,
      double lng,
      Double topLeftLat,
      Double topLeftLng,
    ...
    String wktString = constructWktString(GEOMETRIES.get(whichGeom),
        topLeftLat,
        topLeftLng,
        bottomRightLat,
        bottomRightLng);
    ...
    uniResponse.setDisplaySerial(String.valueOf(topLeftLat)
  ...
  private ArrayList<UniversalFederatedSourceResponse> generateDataForResponse(int amount,
      ...
      results =
          buildObjectsForSource(amount, topLeftLat, topLeftLng, bottomRightLat, bottomRightLng,
              s,e);
  ...
  private void populateArrays() {
    ...
    originateUnit.add(
        "Defense Language Institute Foreign Language Center, Monterey, CADefense Language Institute Foreign Language Center, Monterey, Calif.");
    ...
    originateUnit.add(
        "Post Traumatic Stress Disorder and Mild Traumatic Brain Injury (PTSD/MTBI) Chain Teaching Program");
    originateUnit.add(
    ...
    originateUnit.add(
        "Tank-automotive & Armaments Command (TACOM) Life Cycle Management Command (LCMC) Integrated Logistics Support Center");
